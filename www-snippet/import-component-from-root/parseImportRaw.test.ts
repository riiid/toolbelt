...
Deno.test("parseImportRaw - default export", () => {
  const nodes = parseImportRaw(
    `import ThisDefaultImport from "https://deno.land/x/cliffy@v0.19.1/command/mod.ts";`,
  );
  assertEquals(nodes.at(0)?.importName, "ThisDefaultImport");
});
Deno.test("parseImportRaw - default export as", () => {
  const nodes = parseImportRaw(
    `import * as ThisDefaultImportAs from "https://deno.land/x/cliffy@v0.19.1/command/mod.ts";`,
  );
...
Deno.test("parseImportRaw - named export", () => {
  const nodes = parseImportRaw(
    `import {Command} as ThisDefaultImportAs from "https://deno.land/x/cliffy@v0.19.1/command/mod.ts";`,
  );
